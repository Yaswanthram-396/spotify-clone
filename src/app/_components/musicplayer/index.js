...
export default function MusicPlayer() {
  ...
  useEffect(() => {
    if (audioUrl && !audio) {
      ...
      // Restore current time from localStorage
      const savedCurrentTime = parseFloat(localStorage.getItem("currentTime"));
      if (savedCurrentTime) {
        newAudio.currentTime = savedCurrentTime;
      }
      ...
      newAudio.addEventListener("timeupdate", () => {
        ...
        localStorage.setItem("currentTime", newAudio.currentTime.toString());
      });
  ...
  // Update localStorage when isPlaying changes
  useEffect(() => {
  ...
  const handleSongClick = (song) => {
    ...
    setCurrentSongName(song.track);
    setCurrentSongArtist(song.artist);
    // Reset current time when a new song is clicked
    setCurrentTime(0);
